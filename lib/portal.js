...
// -1 = dental plan, -2 = X-rays/photos), which is far more reliable than nth() across the whole
// page — after several invoices the DOM accumulates stale inputs. Verified + retried.
async function uploadTo(page, slot, files) {
  const stem = path.basename(files[0]).replace(/\.[^.]+$/, '').slice(0, 18);
  const byId = page.locator(`#nx-file-uploader-${slot}-input`);
  for (let attempt = 0; attempt < 3; attempt++) {
    const input = (await byId.count()) ? byId : page.locator('input[type=file]').nth(slot);
    await input.setInputFiles(files, { timeout: 12000 }).catch(() => {});
    await page.waitForTimeout(2200 + 800 * (files.length - 1));
    if (await page.getByText(stem, { exact: false }).count().catch(() => 0)) return true;
  }
  return false;
}
...
async function startClaim(page, cfg) {
  ...
  for (const id of ['#question1ToggleNo', '#question2ToggleNo']) {
    if (await page.locator(id).count()) {
      await page.locator(`${id} label`).first().click({ timeout: 4000 }).catch(() => {});
      await page.waitForTimeout(300);
    }
  }
  ...
  const bank = page.locator('nx-radio', { hasText: P.bankAccountMatch });
  if (await bank.count()) {
    await bank.first().scrollIntoViewIfNeeded().catch(() => {});
    await bank.first().click({ timeout: 6000 });
    await page.waitForTimeout(500);
  }
...
async function addInvoice(page, cfg, inv) {
  ...
  // uploads) the overview can take a while to settle, so wait for idle and retry the click.
  await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
  const addBtn = page.getByRole('button', { name: /add (another )?invoice/i }).first();
  for (let attempt = 0; ; attempt++) {
    try { await addBtn.scrollIntoViewIfNeeded({ timeout: 5000 }); await addBtn.click({ timeout: 12000 }); break; }
    catch (e) { if (attempt >= 2) throw e; await page.waitForTimeout(2500); }
  }
  await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
  await page.waitForTimeout(1500);
  // invoice upload (slot 0) — accepts multiple (invoice + payment proof); verified + retried
  await uploadTo(page, 0, inv.invoiceFiles);
  ...
  // supplementary docs -> uploader slots 1,2,... in requiredDocs order (dental plan, X-rays)
  let fileIdx = 1;
  for (const key of Object.keys(inv.docs || {})) {
    const files = inv.docs[key];
    if (!files || !files.length) continue;
    await uploadTo(page, fileIdx, files);
    fileIdx++;
  }
  ...
  if (await page.locator('#masterDiagnosisCode-0').count()) {
    let ok = false;
    if (inv.reason) ok = await selectDropdown(page, 'masterDiagnosisCode-0', inv.reason).then(() => true).catch(() => false);
    if (!ok) {
      const stillRequired = await page.evaluate(() => document.querySelector('#masterDiagnosisCode-0')?.classList.contains('ng-invalid'));
      if (stillRequired) await selectDropdown(page, 'masterDiagnosisCode-0', /\S/).catch(() => {});
    }
  }
