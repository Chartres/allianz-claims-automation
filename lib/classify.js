...
function classify(parsed, cfg, forceTypeKey) {
  let best = null, bestScore = 0, overridden = false;

  if (forceTypeKey && cfg.treatmentTypes[forceTypeKey]) {
    best = forceTypeKey; bestScore = 99; overridden = true;   // agent/sidecar-provided type wins outright
  } else {
    const hay = ((parsed.items || []).join(' ') + ' ' + (parsed.raw || '')).toLowerCase();
    // pdftotext -layout can inject spaces inside words on some invoices ("O rtodontická",
  ...
  return { typeKey: best, type, confidence: bestScore, overridden, missingDocs, docs };
}
