...
// Ported verbatim from the CLI's lib/policy.js (pure string logic) → browser ESM.
...
export function parse(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const l = lines[i];
    if (/Plan$/.test(l) && l.length < 40) { plan = l; }
    if (isValueLine(l)) continue;
    ...
    benefits.push({ plan, name: l, coinsurance: pct(joined), limit_eur: eVals.length ? eVals[0] : null, remaining_eur: eVals.length > 1 ? eVals[eVals.length - 1] : null, raw: joined });
  }
  return {
