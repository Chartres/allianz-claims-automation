// Enrich a claim with canonical provider, per-invoice category + expected co-insurance, and a
// reimbursement check. Ported from the CLI's lib/enrich.js → browser ESM.
import { canonicalProvider } from './canonical.js';

export function enrich(claim, cfg) {
  ...
  const catFor = (provider) => {
    ...
    for (const [k, v] of Object.entries(provCat)) { const kl = k.toLowerCase(); if (p.includes(kl) || kl.includes(p)) return v; }
  ...
  else { check = 'over'; note = `Reimbursed ${reimb} > expected ${expectedTotal} CZK — verify.`; }

  return { ...claim, invoices, expected_reimbursed: haveAllCoverage ? expectedTotal : null, ratio, shortfall: +(inv - reimb).toFixed(2), check, check_note: note };
}
